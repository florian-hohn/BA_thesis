...
The second Kappa statistic, $\kappa_m$ \cite{kappam}, is a measure that compares against a majority class classifier instead of a 
chance classifier:
...
\begin{equation}
  \kappa_temp = \frac{p_0-p'_e}{1-p'_e},
\end{equation}

where $p'_e$ is the accuracy of the No-change classifier, that is classifier that predicts the last class from the stream. It is 
a simple and useful classifier when the same labels appear together in bursts.
...
The first part of the theoretical work will give an introduction to the Learning Vector Quantization (LVQ), from that later 
the Robust Soft Learning Vector Quantization (RSLVQ) was derived from.
The LVQ is a class of learning algorithms for nearest prototype classification (NPC).It stores a set of appropriatly chosen
...
\begin{figure}
  \includegraphics[width=\linewidth]{LVQ}
