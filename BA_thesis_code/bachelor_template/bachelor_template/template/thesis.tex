      ...
      \begin{itemize}
        ...
        \begin{itemize}
          ...
          \begin{itemize}
            ...
            \item results $\rightarrow$ folder that contains the raw result files from the evaluations, that are then used by the 
            process\_eval\_tables.py file to create the final result files.
            \item custom\_evaluation.py $\rightarrow$ contains code for the evaluation process.
            \item main\_evaluation.py $\rightarrow$ contains the setup code for the evaluation.
            \item process\_eval\_tables.py $\rightarrow$ creates the endresult .csv files and saves them in the result file in the latex folder.
          \end{itemize}
          \item BA\_thesis\_code.sln $\rightarrow$ visual studio solution file for the code.
          \item bachelor\_template $\rightarrow$ contains all the files from the latex files as well as folders for 
          the images and the folder for the endresult from the evaluation in the results folder. 
          
        \end{itemize}
        \item thesis.pdf $\rightarrow$ Pdf file of the thesis
      \end{itemize}
